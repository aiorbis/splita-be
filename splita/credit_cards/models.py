...
class CreditCard(models.Model):
    ...
    credit_limit = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(Decimal('0.01'))])
    current_balance = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    minimum_payment = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))
    ...
    billing_cycle_day = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(31)])
    payment_due_day = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(31)])
    ...
    def total_emi_amount(self):
        """Total EMI amount for all active EMIs of this credit card"""
        return self.emis.filter(status__in=['PENDING', 'OVERDUE']).aggregate(
            total=Sum('emi_amount'))['total'] or Decimal('0.00')
    
    @property
    def emi_count(self):
        """Count of active EMIs for this credit card"""
        return self.emis.filter(status__in=['PENDING', 'OVERDUE']).count()
    
    @property
    def total_remaining_amount(self):
        """Total remaining amount for all EMIs of this credit card"""
        total = Decimal('0.00')
        for emi in self.emis.filter(status__in=['PENDING', 'OVERDUE']):
            total += emi.remaining_amount
        return total
    
    def update_current_balance(self):
        """Update current_balance to match total remaining amount from all EMIs"""
        self.current_balance = self.total_remaining_amount
        self.save(update_fields=['current_balance', 'updated_at'])
    
    def sync_balance_with_emis(self):
        """Sync current_balance with EMI remaining amounts - alias for update_current_balance"""
...
class CreditCardEMI(models.Model):
    """
    Model for Credit Card EMI schedules
    """
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('PAID', 'Paid'),
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='credit_card_emis')
    credit_card = models.ForeignKey(CreditCard, on_delete=models.CASCADE, related_name='emis')
    emi_amount = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(Decimal('0.01'))])
    total_emi_amount = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), help_text="Total EMI amount (emi_amount * total_duration)")
    ...
    def save(self, *args, **kwargs):
        """Override save to calculate total_emi_amount automatically and sync credit card balance"""
        if not self.total_emi_amount:
            self.total_emi_amount = self.emi_amount * self.total_duration
        ...
        # Update credit card current_balance to match EMI remaining amounts
        self.credit_card.update_current_balance()
    
    def __str__(self):
        return f"EMI - {self.credit_card} - {self.start_date} to {self.end_date}"
    ...
    def remaining_amount(self):
        """Calculate remaining amount as total_emi_amount - (completed_months * emi_amount)"""
        return self.total_emi_amount - (self.completed_months * self.emi_amount)
    ...
    def mark_as_paid(self, amount=None, payment_date=None):
        """Mark EMI as paid and update credit card balance"""
        if amount is None:
        ...
        self.save()  # This will trigger credit card balance update through save() method
...
class CreditCardPayment(models.Model):
    """
    Model to track payments made towards Credit Card EMIs
    """
    PAYMENT_METHODS = [
        ('CASH', 'Cash'),
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='credit_card_payments')
    emi = models.ForeignKey(CreditCardEMI, on_delete=models.CASCADE, related_name='payments')
    ...
    def save(self, *args, **kwargs):
        """Override save to sync credit card balance when payment is made"""
        super().save(*args, **kwargs)
        
        # Trigger credit card balance update
        self.emi.credit_card.update_current_balance()
...
class CreditCardEMIReminder(models.Model):
    """
    Model for Credit Card EMI reminders and notifications
    """
    REMINDER_TYPES = [
        ('EMAIL', 'Email'),
        ('SMS', 'SMS'),
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='credit_card_emi_reminders')
    emi = models.ForeignKey(CreditCardEMI, on_delete=models.CASCADE, related_name='reminders')
